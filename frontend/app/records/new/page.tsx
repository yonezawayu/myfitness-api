...
import { createRecord, TOKEN_STORAGE_KEY } from "@/lib/api";
...
export default function NewRecordPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!weightValue || weightValue <= 0 || Number.isNaN(weightValue)) {
      setError("weightは0より大きい数値で入力してください。");
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Recordの保存に失敗しました。");
    ...
    <main className="min-h-screen px-5 py-8">
      <div className="mx-auto max-w-xl">
        <header className="mb-8 border-b border-gray-200 pb-6">
          ...
          <h1 className="mt-2 text-2xl font-semibold text-gray-950">体重記録を追加</h1>
        </header>
